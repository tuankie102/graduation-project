...
const ApplyModal = (props: IProps) => {
    ...
    const handleOkButton = async () => {
        ...
        if (!isAuthenticated) {
            ...
            if (postDetail) {
                const res = await callCreateResume(urlCV, postDetail?.id, user.email, user.id);
                if (res.data) {
                        ...
                        <ConfigProvider locale={enUS}>
                            ...
                            >
                                <Row gutter={[10, 10]}>
                                    <Col span={24}>
                                        <div>
                                            Bạn đang ứng tuyển công việc <b>{postDetail?.job?.name} </b>tại  <b>{postDetail?.job?.company?.name}</b>
                                        </div>
                                    </Col>
                                    <Col span={24}>
                                        <ProFormText
