...
import { callCreatePost, callFetchPostById, callUpdatePost, callFetchJob } from "@/config/api";
...
const ViewUpsertPost = (props: any) => {
    ...
    const onFinish = async (values: any) => {
        if (dataUpdate?.id) {
            ...
            const res = await callUpdatePost(post, dataUpdate.id);
            if (res.data) {
                        ...
                        submitter={
                            {
                                searchConfig: {
                                    resetText: "Hủy",
                                    submitText: <>{dataUpdate?.id ? "Cập nhật bài đăng" : "Tạo mới bài đăng"}</>
                                },
                        ...
                        <Row gutter={[20, 20]}>
                            ...
                            <Col span={24} md={12}>
                                <ProFormDatePicker
                                    label="Ngày bắt đầu"
                                    name="startDate"
                                    normalize={(value: string | Date) => value && dayjs(value, 'DD/MM/YYYY')}
                                    fieldProps={{
                                        format: 'DD/MM/YYYY',
                                    }}
